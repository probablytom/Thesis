...
chaptes and offer important background for the research undertaken in it.
...
\begin{enumerate}
    ...
    \lstinline{objA.__eq__(objB)}. Many such magic methods exist. These methods
    can be overridden or specified by a programmer.
\end{enumerate}
...
the \lstinline{fuzz_function} function on it. Fuzzing is dynamically applied, as
advice is retrieved and applied only when a target is invoked and its prelude
advice is executed.

The \lstinline{fuzz_function} function retrieves the abstract syntax tree
...
change afterwards.\footnote{Later prelude advice or the initial stages of around
