...
class WorkflowTransformer(ast.NodeTransformer):
    def __init__(self, fuzzer=lambda x: x, strip_decorators=True, context=None):
        """
        :param fuzzer: a function that takes a list of strings (lines of program
            code) and returns another
        list of lines. :param strip_decorators: removing decorators prevents
        re-mutation if a function decorated with a mutator is called
        recursively.
        """
