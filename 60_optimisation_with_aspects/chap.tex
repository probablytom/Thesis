...
%% === 28-11-23 The following is a rewrite of the above intro, to make this
%      chapter specific to the _setup_ of the experiments. Results will follow
%      in the next chapter. Not quite finished writing it yet.

This chapter describes the design and implementation of experiments
...
The use of aspect-oriented programming in previous simulation and modelling
efforts have typically focused on the use of aspects to compose model or
simulation details\inline{There must be tons of good citations for aspects being
used to compose together a simulation / model}\revnote{We don't think this is
the case --- those citations just don't exist! Rework to discuss use of AOP in
business process modelling, and note that nobody made those models in practice,
just tooling.}\revnote{Also, just read this and noted that while AOP isn't
necessarily used for model composition, there \emph{are} lots of projects for
modelling via composition, just not using AOP. Although perhaps I can just cite
tooling here, and ancillary projects like aspect-oriented lindenmeyer
systems\ldots{}?}. Critics of aspect orientation note that the act of oblivious
...
introduction of aspect orientation. \inline{this last sentence doesn't make much
sense to me when reviewing.} However, aspect orientation might instead be used
...
\inline{
  I've changed the RQs in the above list, but am currently restructuring this
  chap, so yet to edit this section and make sure it makes sense with the
  reworded Qs. Make sure it does!
}
...
\inline{
  This paragraph explains the structure of the first experimental chapter, but
  if it's not out of date as I'm writing this, it's about to be. Revisit when
  I'm not restructuring lots of things! Also, the things they refer to are now
  out-of-order and sometimes subsections instead of sections. Needs re-doing,
  basically.
}
...
% \inline{We've set the stage to explain and justify our own models of learning.
% Let's go!}
...
The first --- that players should make observations about winning players ---
...
Such a model of confidence offers a model of learning when combined with a PMF
