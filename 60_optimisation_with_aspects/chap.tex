...
\laurie{i found this sentence a bit difficult to parse (it is correct, valid, English, but it might be a bit too terse)} The behaviour modified by the advice is players' selection of character pairs.
...
established. The results in \cref{sec:rq2} and \cref{sec:rq3} are positive. \laurie{If you make this sentence ``Only RQ4 remains to be answered'' then I think it's clear that it's a) not a rhetorical question b) remind readers that they saw the question earlier. two birds, one etc etc} A
final research question remains, which is: \emph{\rqfour{}}\revnote{It occurs to
me that these \emph{might} read as rhetorical questions when used in this way.
I'm not sure so I'd love somebody to review the chapter and let me know how it
comes across, so I can change this if necessary. I'm struggling to see the forest for
...
topic by \cref{howard1971metagames_seminal} \laurie{should be `cite' not `cref'?}.
...
parlance\inline{reference pdsf's explanatory chapter / re-engineering chapter
here? unsure which is most appropriate / introduces the concepts first.}).
...
\label{rq1_answered}
Having defined the advice used by the models, we are now able to answer the
...
\inline{
  First RQ needs addressing here, as we've built the aspects which represent
  behavioural variations and so can show what those changes to a model look like
  as advice
  WRITE ME
  WRITE ME
  WRITE ME
  ...
  WRITE ME
  WRITE ME
  Also consider changing the phrasing of RQ1?
}
...
\begin{figure}[tp]
    \centering
    \lstinputlisting{60_optimisation_with_aspects/snippets/ModelParameters.py}
