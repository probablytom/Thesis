...
\correctionnote{
    I now think this is untrue: aspectj can only weave hooks at load time to
    whichever class is being loaded, not a delegate and not a parent. Delegates
    appear to be imports kicked off by the current classloader. So, the example
    config only weaves hooks into the packages at the starred level of nesting.
    It's a little too late for me to confirm that currently. Revisit in the
    morning.
}
