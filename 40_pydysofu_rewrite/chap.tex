...
support officially ended during this PhD). \inline{Lose the brackets} \inline{Be
less clumsy about py2 support}
...
\inline{
    This footnote should be moved into the body of this text.
}
...
ForbiddenFruit~\cite{forbiddenfruit_repo}. \inline{Explain the ForbiddenFruit
technique properly --- lots of work done so worth explaining fully. In
particular, explain in detail why it's not viable.} Efforts were made to add the
...
\inline{
    This should be rewritten. Tim suggests having separate implementation and
    application sections. There's not \emph{loads} of content here already but
    adding extra to have something specifically on use could make sense. There's
    more detail to add: I don't specify how weaving actually takes place, 
}

To weave aspect hooks, \pdsf{} must modify \lstinline{builtins.__import__} only
