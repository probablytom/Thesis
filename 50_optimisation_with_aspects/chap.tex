...
\section{Experimental Design using Aspectual Learning Models}\label{sec:optimisation_with_aspects_experimental_design}
...
% The research questions which these experiments aim to answer require a method for comparing the similarity
% of character pair selection in a given instance of simulated play. Quantifying how similar a simulated
% player's character pair selections are to their real-world counterpart allows such a comparison.
% The goal of this comparison is to determine whether synthetic datasets of character pair choice data align
% closely with empirically sourced datasets insofar as this comparison investigates whether real-world players
% are more accurately modelled in one simulation than another; 

% What do I want to say here?
% 
% * We need to compare distributions of choices rather than choice history because
%   we'd expect natural variations, and what matters is whether
%   players ended up choosing similarly as they learned
% * A distribution gives us a ranking of choices; we can see preferences. We're therefore interested
%   in whether players consistently ranked certain character pairs over others.
% * There's lots of RNG in RPGLite, so the degree to which those character pairs are picked
%   is a tricky thing to get right. It's also not necessarily a good measure of similarity: whether
%   players learn can be measured in a bunch of ways, and similar _proportions_ is a harder bar
%   to reach than similar rankings.
% * It matters that we're not interested in similar proportions because it changes the correlation
%   metrics we use to measure. Our aim is to show that the aspect-applied model is "more realistic".
%   We can decide what that means. If players get the character pair preferences right, surely that's
%   enough for us.
% * We can't predict much about our data because different players might have different properties
%   to their character pair choices, _but_ we can make zero assumptions about the data by using
%   an appropriate technique to calculate similarity. Non-parametric rank coefficients tell us
%   how closely associated two rankings are. Two major choices are Spearman and Kendall; we opt for
%   Kendall because Spearman's is sensitive to discrepancies between rankings, rather than simply whether
%   the rankings are concordant. Character pairs which aren't chosen often are likely to have large
%   discrepancies in rank but small discrepancies in their rate of choice, and we shouldn't penalise
%   a dataset for having similar rates of choice but dissimilar rankings in the tail of the distribution of
%   character pair choice.
% * We can get the pvalue of two datasets being associated directly from scipy's implementation of
%   Kendall's Tau; so, we can use the pvalue of the datasets being associated as our similarity metric.

% * We need to compare distributions of choices rather than choice history because
%   we'd expect natural variations, and what matters is whether
%   players ended up choosing similarly as they learned

The experiments presented in this chapter make comparisons of many synthetic datasets to a single
...
distribution's rank correlation.\inline{Should I explain rank correlation more where it's first introduced\ldots{}?}

Many rank correlation measurements exist; \inline{Briefly discuss the different rank correlation measurements
I could have chosen.}
...
\inline{Stopped discussing rank correlation here because I may re-run my experiments; if I do, I don't want
to have to re-write this, because frankly it's been pretty tricky to write so far and I'm keen not to have to
redo it. Maybe there are other correlation metrics we could pick instead; Somers' D looks promising…? Are
there citations I could use that compare different measurements, so I could justify my choice, and maybe pick
one that's better than \tau if it exists?}
...
%   a dataset for having similar rates of choice but dissimilar rankings in the tail of the distribution of
%   character pair choice.
% * We can get the pvalue of two datasets being associated directly from scipy's implementation of the
%   metric, and use that to compare similarity directly; the pvalue will tell us the chance that two
%   datasets are similar by chance, which is effectively what we want to minimise when fitting a model of
%   learning to a given player.


\subsection{Annealing toward Player-Specific Learning Parameters}

\inline{Discussion required here around the mitigation of randomness --- it's required because of RPGLite's
inherent RNG --- and also explaining how we make use of a grid search in combination with
k-fold validation to anneal toward well-fitted parameters of our models of learning for each individual player.}
