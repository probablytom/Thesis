...
season 1 are marked \emph{N/A}, as no parameters are available to run season
2 simulations with. 
...
or add additional behaviour. However, \cref{sec:rq4} fails to demonstrate this
for advice which introduces new behaviours and parameters to a model. These
findings may be biased by players' season 1 habits around character pair
selection being carried into their season 2 behaviour. If players chose
characters habitually rather than responding to RPGLite's changed metagame, no
learning would be present in their gameplay data.

One player, \emph{Ellen}, was modelled accurately in these experiments, and was
the only player accurately modelled both with parameters which produced accurate
data in season 1 and with parameters generated using k-fold validation on season
2 data. The model of learning therefore works for season 2, but does not model
players successfully in this season. It is possible that the design of the
learning model represents players' initial learning of the game, but represents
their response to a changing environment poorly, thereby modelling play of the
first season more accurately than the second.
% This amounts to a type of ``knowledge'' about the system advice is woven into,
% as the system it is applied to may not be an \emph{appropriate} target for the
% advice. 

The fourth research question can therefore be affirmatively answered for
\emph{some} aspects, though more research into the portability of complex
aspects is required. The experiments described in \cref{sec:rq4} show that
simple changes to one model can be applied to similar models. Theoretically,
this result holds when weaving into any alternative model for which the
aspectually-represented variation is appropriate --- more research is required
to confirm this. The result could be confirmed in this broader sense by creating
...
Though aspects representing behaviours and parameters may be demonstrated to
model a system accurately, \cref{sec:rq4} does not demonstrate conclusively that
they may be reused in another model reliably, and further research would also
clarify whether RPGLite's players were biased by their season 1 experiences or
whether the model of learning was semantically inappropriate for application to
a model of RPGLite season 2 on a more fundamental level.
