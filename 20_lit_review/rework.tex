...
order~\revnote{don't use a description list here, this should be a regular paragraph and also explain \emph{why} each section is included.}:
...
\revote{Would suggest the following flow here: 1. modularity is considered important for maintainability. 2. Common approaches such as OO do not address cross cutting concerns such as authentication and logging well. Once you've done that you can then combine some of the arguments as second sentence of second paragraph otherwise feels somewhat repetitious.}
...
many industrially-relevant languages today. \inline{Citation needed?}\revnote{Suggest citing some languages that are OO}
...
(usually the moment of invocation or return of some function or method).
    \item \emph{``Advice''} defines some behaviour, such as emitting a logline, which \revnote{Do you mean outputting a log message}
...
\revnote{The blockquote environment appears broken as it only quotes the first character.}
\begin{blockquote}
...
\revnote{Tighten the phrasing here: say it is unclear from the existing literature whether aspect-oriented approaches really do achieve these goals as there is a general lack of empirical evidence or even case studies in order to make the assessment cite, cite. However, to do so is not in the scope of the current work. }
...
\revnote{Feels like the full literature on AOP is missing here.  What is the available evidence that it works, if any? What has it been used for? Some sort of survey of AOP tech might also be useful for talking about alternatives?}
...
\revnotes{ citet command is a neater way of achieving citeauthor/cite}

\revnote{this needs more explanation.  What were Kiczales et al. trying to do, what was the context, what did they demonstrate?}
...
functionality, though they achieve this through their reflective qualities and \revnote{reflective qualities?  perhaps reflective programming techniques?}
...
different functionality to a software engineer. Engineering patterns such as \revnote{feels like a new paragraph on decorators}.
...
\revnote{does this come before the other approaches section.}
...
regards: \revnote{not sure this needs enumerating}
...
\revnote{I would rephrase the opening of this as direct report of voice to make it more succinct.  \cite{Constantinides04aopconsidered} offered an early critique of AOP, making unfavourable comparisons with criticisms of GOTO statements.}
...
\revnote{Again, direct active report of voice is more succinct. \citeauthor{steimann06paradoxical} makes a similar, but more thorough critique of AOP.}

\revnote{Generally, avoid referring to a ``paper'' refer to the authors.}
...
\revnote{The style in the below is what I'm suggesting}
...
\revnote{I think this comes before the critique section perhaps?  Need a general introduction to AOP techniques and techs, including this?}
...
\revnote{Need citations to these technologies}
...
\revnote{My worry in below is that it feels like you are teasing the reader - well WHAT environments/contexts then, huh?}
...
(because they require no direct support), and in their ability to represent \revnote{Long sentence and not sure what you are trying to say exactly}
...
A very early example of aspect orientation in simulation \& modelling is\revnote{Can you be more precise here? Is it the first instance?}
...
simulation of agent-based models\revnote{Don't think Swarm has been mentioned before so needs explanation/cite - sentence or two}.%
Their aspect orientation effectively makes use
of Observer patterns to measure a pre-constructed system under simulation,
without the observations being an intrinsic component of the simulated system.\revnote{intrinsic? explicit?}
...
\citeauthor{gulyas1999use}'s work presents not only tooling for \revnote{simplify ``not only'' here - why deprecate tooling?}
...
reduction of tangling \& scattering. \revnote{Could simplify as ``Gulyas presents additional arguments for adopting AOP in modelling and simulation''?}
In particular, their work discusses
...
\cite{kiczales1997aspect}.\revnote{it seems like this might be an important exposition, so perhaps more details?}
The fact that a rare example of detailed reasoning
...
premise this thesis shares. \revnote{This is waffly.  Would suggest simplifying to something like ``The position adopted by Gulyas aligns with that taken in this thesis: that aspect orientation has signficant benefits for supporting software based modelling and simulation''}
...
early in the lit review. They don't belong buried so far down in this chapter.}\revnote{Agree}
They propose that aspect orientation solves these problems, and identify that
there are potential benefits in discrete event simulation \revnote{(DES)} code in both regards,
...
is the discussion of AOP's potential application to DES codebases\revnote{repetition}, and detail of
...
aspect oriented enhancements they identify. \revnote{Be explicit about what Japrosim is - is it a DESF?}

\revnote{Just say ``Later,...,''} In a later publication~\cite{chibani2019using}, \citeauthor{chibani2019using}
identify\revnote{generally past tense preferred} opportunities for the use of aspect orientation in simulation tooling,
...
the design itself. Counting\revnote{Evaluating?} the main improvements between the presented
aspect-oriented design and the existing object-oriented one is left to future
work\revnote{new sentence here - \citet{chibani2019using}'s later work presented an implementation of their proposal and provide a quantitative evaluation.} %
...
\revnote{simplify - Similar to}In a manner similar to
...
to be found\revnote{simplify - observed that their are advantages to adopting AOP} in a simulation framework able to take advantage of aspect
...
improves modularity and maintainability of a codebase.\revnote{I'd avoid handwavy conclusions like this.  Say something more sober like ``In general, the AOP for modelling and simulation literature defers to claims in general AOP literature that AOP enhances...}
...
\citeauthor{DEVSaspectorientation2008aksu} detail\revnote{provide} a case study of their
...
\revnote{Simplify - The work therefore provides evidence that AOP is suitable for supporting simulation and modelling in real-world ontexts and use case. Several authors have otherwised criticised the lack of practical evaluation of AOP generally \cite{}}
...
\revnote{Generally remove ``note'' prefixes to sentences. ``Improvements to code quality...''}
...
processes are inherently sociotechnical\revnote{use macro?} and this thesis presents tooling for and
...
section here}\renote{Agree, builds well from preceding sections though}
...
presumably \revote{why only presumably?} only permissible when representable through the composition of the
